 ...
 */
float* vector_scalar_multiply(float* vector, int vector_size, float scalar){
    for (int i = 0; i < vector_size; ++i) {
        vector[i] *= scalar;
    }
    return vector;
...
float* get_contribution_rates(float* eignvalues, int number_of_sensors){
    float* contribution_rates = malloc(number_of_sensors * sizeof(float));
...
float* get_cumulative_contributions(float* contribution_rates, int number_of_sensors){
    float* cumulative_contributions = malloc(number_of_sensors * sizeof(float));
    for (int i = 0; i < number_of_sensors ; ++i) {
...
float* get_integrated_support_score(float** principal_components, float* contribution_rates, int number_of_sensors){
    float* integrated_scores = malloc(number_of_sensors * sizeof(float));
    for (int i = 0; i < number_of_sensors; ++i) {
        float* score = vector_scalar_multiply(principal_components[i], number_of_sensors, contribution_rates[i]);
